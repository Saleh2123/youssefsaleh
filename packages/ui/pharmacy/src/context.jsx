...
const AppProvider = ({ children }) => {
  ...
  const [chosenAddress,setChosenAddress] = useState({
    ...
    const [addresses,setAddresses] = useState([
      ...
      const [prescriptions,_setPrescriptions] = useState([
        {
          medicineName: 'MedicineN',
          dose: 10,
          doctorName: 'Ahmed Sherif',
          date: new Date()
        },
        {
          medicineName: 'MedicineE',
          dose: 15,
          doctorName: 'Ahmed Mostageer',
          date: new Date()
        },
        {
          medicineName: 'MedicineC',
          dose: 12,
          doctorName: 'Mohamed Diab',
          date: new Date()
        }
        ]);
  ...
  const CountMedicineInCartTwo = (medicine,Cart)=>{
    let count = 0;
    for(let i = 0 ; i < Cart.length;i++){
      if(Cart[i].name === medicine.name){
        count ++;
  ...
  const [uniqueMedicines,setUniqueMedicines] = useState(groupMedicines(cart));

  useEffect(()=>{
     setUniqueMedicines(groupMedicines(cart));
  },[cart])

  const [Allmedicines, setAllMedicines] = useState([]);
  const [medicines, setMedicines] = useState([]); //dummy data
  ...
  useEffect(() => {
    const get = async () => {
      const results = await _trpc.vanilla.medicine.find.query();
